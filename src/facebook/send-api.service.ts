...
export class SendApiService {
  ...
  public async sendTextMessage(
    ...
    try {
      ...
      this.logger.log(
        `Message sent with status ${JSON.stringify(facebookResponse.data)}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `Method: ${this.sendTextMessage.name}, cannot send text message, the response is ${error.message}`,
      );
      return false;
    }
  }
  ...
  public async sendAttachment(
    ...
    try {
      ...
      this.logger.log(
        `Attachment sent with status ${JSON.stringify(facebookResponse.data)}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
  ...
  public async sendTypingAction(request: SendActionRequest): Promise<boolean> {
    try {
      ...
      this.logger.log(
        `Sending typing action: ${JSON.stringify(facebookResponse.data)}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
